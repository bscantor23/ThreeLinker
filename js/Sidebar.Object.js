...
function SidebarObject( editor ) {
	...
	*/
	...
	const objectMatrixAutoUpdateRow = new UIRow();
	const objectMatrixAutoUpdate = new UICheckbox().onChange( update );

	objectMatrixAutoUpdateRow.add( new UIText( strings.getKey( 'sidebar/object/matrixAutoUpdate' ) ).setClass( 'Label' ) );
	objectMatrixAutoUpdateRow.add( objectMatrixAutoUpdate );

	container.add( objectMatrixAutoUpdateRow );

	// layers

	const objectLayersRow = new UIRow();
	const objectLayers = new UIInteger( 1 ).setWidth( '50px' ).onChange( update );

	objectLayersRow.add( new UIText( strings.getKey( 'sidebar/object/layers' ) ).setClass( 'Label' ) );
	objectLayersRow.add( objectLayers );

	container.add( objectLayersRow );
	...
	function update() {
		...
		if ( object !== null ) {
			...
			if ( object.matrixAutoUpdate !== undefined && object.matrixAutoUpdate !== objectMatrixAutoUpdate.getValue() ) {

				editor.execute( new SetValueCommand( editor, object, 'matrixAutoUpdate', objectMatrixAutoUpdate.getValue() ) );

			}

			if ( object.layers !== undefined && object.layers.mask !== objectLayers.getValue() ) {

				object.layers.mask = objectLayers.getValue();
				editor.execute( new SetValueCommand( editor, object, 'layers', object.layers ) );
	...
	function updateRows( object ) {
		...
		const properties = {
			...
			'matrixAutoUpdate': objectMatrixAutoUpdateRow,
			'layers': objectLayersRow
		};
	...
	function updateUI( object ) {
		...
		if ( object.matrixAutoUpdate !== undefined ) {

			objectMatrixAutoUpdate.setValue( object.matrixAutoUpdate );

		}

		if ( object.layers !== undefined ) {

			objectLayers.setValue( object.layers.mask );
